// This file was generated by [ts-rs](https://github.com/Aleph-Alpha/ts-rs). Do not edit this file manually.
import type { AgentId } from "./AgentId";
import type { CardId } from "./CardId";
import type { CardLink } from "./CardLink";
import type { CardPriority } from "./CardPriority";
import type { ChecklistItem } from "./ChecklistItem";
import type { ColumnId } from "./ColumnId";
import type { TeamId } from "./TeamId";

/**
 * Um cartão com o que a leitura precisa ao lado (handles, dependências abertas).
 */
export type CardView = { columnSlug: string, assigneeHandle: string | null, 
/**
 * Dependências ainda abertas.
 */
blockedBy: Array<CardId>, comments: number, id: CardId, teamId: TeamId, columnId: ColumnId, title: string, 
/**
 * Markdown.
 */
body: string, assignee: AgentId | null, 
/**
 * `None` = você ou o sistema.
 */
createdBy: AgentId | null, parentId: CardId | null, position: number, priority: CardPriority, labels: Array<string>, checklist: Array<ChecklistItem>, links: Array<CardLink>, blockReason: string | null, 
/**
 * Trava otimista: toda gravação sobe um. `claim` só vale sobre a versão lida.
 */
version: number, archivedAt: number | null, approvedBy: AgentId | null, approvedAt: number | null, 
/**
 * Desde quando está na coluna atual (`card_stale`, "há 18min").
 */
columnSince: number, createdAt: number, updatedAt: number, };
